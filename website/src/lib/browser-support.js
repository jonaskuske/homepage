...
export const isFirebug = window.console && (window.console.firebug || (window.console.exception && window.console.table)) && true;
...
if (isIE) {
  ...
  const noticeText = domainExtension === 'com' ? 'You are using a dated browser. Consider upgrading to a modern browser like Edge, Chrome or Firefox for better security and an enhanced experience! (Click to hide)' : 'Internet Explorer ist veraltet. Nutze für mehr Sicherheit und ein besseres Surferlebnis einen modernen Browser wie Edge, Chrome oder Firefox! (Klicken zum Schließen)';
  ieNotice.textContent = noticeText;
  ieNotice.setAttribute('style', 'position:fixed;width:100%;top:0;z-index:99;margin:0;padding:5px;cursor:pointer;text-align:center;background:rgba(0,0,0,0.6);font-size:0.8rem;');
...
* @type {boolean}
*/
const consoleStylingSupported = (isWebkit && !isEdge) || isFirebug || isFirefoxWithLogStyleSupport || false;

const objectFitSupported = window.CSS && window.CSS.supports && window.CSS.supports('(object-fit: contain)');
