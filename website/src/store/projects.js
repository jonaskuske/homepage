...
export const projectActions = {
  ...
  fetchProjects: () => async ({ language }, actions) => {
    const {
      default: projects,
    } = await import(/* webpackChunkName: "projects/[request]" */ `@/assets/${language}-projectlist.js`)
  ...
  requestProject: id => async ({ projects, language }, actions) => {
    ...
    const { default: project } = await import(/* webpackChunkName: "projects/[request]" */
    `@/assets/projects/${id}/${language}-assets`).catch(e => {
      log(e)
