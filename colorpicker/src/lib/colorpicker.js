/* eslint-disable quotes, no-extra-semi */
  ...
  function Plugin($container, options) {
    ...
    function _setImage() {
      ...
      colorPicker.onload = function() {
        const height = window.innerHeight * 0.77
    ...
    function _getColorCanvas(event) {
      if (mouseIsDown) {
        var offset = event.target.getBoundingClientRect(),
          colorData = context.getImageData(
        ...
        $container.dispatchEvent(changeEvent, [self.hex, self.rgb])
      }
    }
    ...
    this.setColor = function(color) {
      ...
      $colorInner.style.backgroundColor = self.hex
      $colorInput.value = self.hex
    }
